...
var dshwDesktopMode = window.__dshWhaleDesktop === true
...
try {
  ...
  // 初始已有 composer → 主界面
  if (dshwDesktopMode || dshwIsChatRoot(dshwRoot)) {
    dshwEnabled = true
  } else {
...
var HEADPHONES_IMG_URL = '/dsh-whale/image-headphones.png?v=2'
var TSUN_IMG_URL = '/dsh-whale/image-tsun.png?v=2'
var TSUN_AUDIO_URL = '/dsh-whale/tsun.mp3'
var EAT_IMG_URL = '/dsh-whale/image-eat.png?v=2'
...
var desktopBubbleResumeTimer = null
...
bubbleBox.addEventListener('click', function (e) {
  ...
  if (tsunActive || eatActive || (bubbleScene && bubbleScene.kind === 'desktop-special')) return
  if (audioPlaying && bubbleScene && bubbleScene.kind === 'desktop-music') {
    showBubble()
    return
  }
  if (costBubbleActive) {
...
// —— 桌面壳扩展：音乐形态 + 两个本地彩蛋 ——
function desktopIdleImageUrl() {
  try { return currentRole && currentRole.url ? currentRole.url : IMG_URL } catch (err) { return IMG_URL }
}
function updateWhaleImage() {
  var url = tsunActive ? TSUN_IMG_URL : (eatActive ? EAT_IMG_URL : (audioPlaying ? HEADPHONES_IMG_URL : desktopIdleImageUrl()))
...
function showDesktopLineBubble(text, kind, ttlMs) {
  if (!bubbleOn) return
  bubbleRoundOn = false
  sceneOpen(kind, function () {
    bubbleRenderModules([{ type: 'text', text: text, size: 12, bold: true, color: '#536ba9' }])
  }, ttlMs || 0)
}
function showMusicBubble() {
  if (!audioPlaying || tsunActive || eatActive || !bubbleOn) return
  showDesktopLineBubble('♪ 哼哼~', 'desktop-music', 0)
}
function scheduleMusicBubble() {
  if (desktopBubbleResumeTimer) { clearTimeout(desktopBubbleResumeTimer); desktopBubbleResumeTimer = null }
  if (!audioPlaying || tsunActive || eatActive || !bubbleOn) return
  desktopBubbleResumeTimer = setTimeout(function () {
    desktopBubbleResumeTimer = null
    if (audioPlaying && !tsunActive && !eatActive && !bubbleShown) showMusicBubble()
  }, 260)
}
function clearDesktopSystemBubbles() {
  try { whaleSysQueue = [] } catch (err) {}
  try { whaleSysItem = null } catch (err) {}
  try { if (whaleSysTimer) { clearTimeout(whaleSysTimer); whaleSysTimer = null } } catch (err) {}
  costBubbleActive = false
}
function registerTsunClick() {
  if (!dshwDesktopMode) return false
  if (tsunActive || eatActive) return true
  var now = Date.now()
  tsunClicks.push(now)
  while (tsunClicks.length && now - tsunClicks[0] > TSUN_WINDOW_MS) tsunClicks.shift()
  if (tsunClicks.length < TSUN_CLICKS) return false
  tsunClicks = []
  triggerTsun()
  return true
}
function triggerTsun() {
  ...
  clearDesktopSystemBubbles()
  updateWhaleImage()
  showDesktopLineBubble('杂鱼~杂鱼~', 'desktop-special', TSUN_MS)
  try {
    if (window.__whaleSelfAudioStarted) window.__whaleSelfAudioStarted(4000)
    var audio = new Audio(TSUN_AUDIO_URL)
    audio.play().catch(function () {})
  } catch (err) {}
  if (tsunRevertTimer) clearTimeout(tsunRevertTimer)
  tsunRevertTimer = setTimeout(function () {
    tsunRevertTimer = null
    tsunActive = false
    root.classList.remove('dshwv-tsun')
    updateWhaleImage()
    if (audioPlaying && !bubbleShown) showMusicBubble()
  }, TSUN_MS)
}
function triggerEat() {
  if (tsunActive || eatActive) return
  ...
  clearDesktopSystemBubbles()
  updateWhaleImage()
  showDesktopLineBubble('啊呜~狂吃 token 中~', 'desktop-special', EAT_MS)
  try {
    if (window.__whaleSelfAudioStarted) window.__whaleSelfAudioStarted(4000)
    var audio = new Audio(EAT_AUDIO_URL)
    audio.play().catch(function () {})
  } catch (err) {}
  if (eatRevertTimer) clearTimeout(eatRevertTimer)
...
function setAudioPlaying(playing) {
  ...
  root.classList.toggle('dshwv-music', playing)
  updateWhaleImage()
  if (playing) {
    if (!tsunActive && !eatActive && !bubbleShown) showMusicBubble()
  } else if (bubbleScene && bubbleScene.kind === 'desktop-music') {
    hideBubble()
  }
}
  ...
  fetch(BALANCE_URL + (manual ? '?refresh=1' : ''), { cache: 'no-store', signal: ctrl ? ctrl.signal : undefined })
    ...
    .then(function (data) {
      if (data && data.ok) {
        ...
        if (!tsunActive && !eatActive) checkUsageAlerts(nb, state.todayUsage)
...
function setBubbleOn(v) {
  ...
  else if (audioPlaying && !bubbleShown) showMusicBubble()
}
...
function endDrag(e, clickAllowed) {
  ...
  if (clickAllowed && !drag.moved) {
    ...
    if (registerTsunClick()) return
    if (audioPlaying && bubbleScene && bubbleScene.kind === 'desktop-music') showBubble()
    else whaleClick()
    refresh(true)
    return
  }
