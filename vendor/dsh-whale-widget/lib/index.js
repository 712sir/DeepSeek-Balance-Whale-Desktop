...
export default {
  ...
  apply(ctx) {
    ...
    try {
      const codexWarmTimer = setTimeout(function () { try { codexSummaryCached() } catch (err) {} }, 1500)
      const codexRefreshTimer = setInterval(function () { try { codexSummaryCached() } catch (err) {} }, 5 * 60 * 1000)
      if (codexWarmTimer && typeof codexWarmTimer.unref === 'function') codexWarmTimer.unref()
      if (codexRefreshTimer && typeof codexRefreshTimer.unref === 'function') codexRefreshTimer.unref()
      disposers.push(() => {
        clearTimeout(codexWarmTimer)
